#+begin_src emacs-lisp :tangle lisp/common.el
  ...
  (setq user-full-name "Richard Hunter"
        user-mail-address "richard.hunter@hpe.com")
#+end_src
...
** Ibuffer
I prefer Ibuffer to buffer window. Pretty colors and such. The keybindin =C-b= is overridden to open Ibuffer instead of vanilla buffer window.
#+begin_src emacs-lisp :tangle lisp/hooks.el
(use-package ibuffer
  :config
